...
class NYUDataset(Dataset):
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, (pred, index) in enumerate(zip(preds, indices)):
            ...
            # img_name = strs[-2] + '_' + strs[-1]
            # gt_output_file = os.path.join('viz_depth/nyud', 'gt', img_name)
            # viz_gt.save(gt_output_file) 

            img_name = strs[-2] + '_' + strs[-1].split('.jpg')[0] + '_ours_' + str(round(rmse,4)) +'.jpg'
            pred_output_file = os.path.join('viz_depth/nyud', 'ours', img_name)
