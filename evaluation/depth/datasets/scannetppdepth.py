...
class ScanNetPPDepthDataset(Dataset):
    ...
    def pre_pipeline(self, results):
        ...
        results["cam_intrinsic"] = [
            ...
            [5.1946961112127485e02, 0, 2.5373616633400465e02],
            [0, 0, 1],
        ]
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, (pred, index) in enumerate(zip(preds, indices)):
            ...
            # img_name = strs[-3] + '_' + strs[-1].replace('JPG','jpg')
            # gt_output_file = os.path.join('viz_depth/scannetpp', 'gt', img_name)
            # viz_gt.save(gt_output_file) 

            # img_name = strs[-3] + '_' + strs[-1].split('.JPG')[0] + '_ours_' + str(round(rmse,4)) +'.jpg'
            # pred_output_file = os.path.join('viz_depth/scannetpp', 'ours', img_name)
