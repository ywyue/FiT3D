...
class ScanNetDepthDataset(Dataset):
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, (pred, index) in enumerate(zip(preds, indices)):
            ...
            # img_name = strs[-3] + '_' + strs[-1]
            # gt_output_file = os.path.join('viz_depth/scannet', 'gt', img_name)
            # viz_gt.save(gt_output_file) 

            img_name = strs[-3] + '_' + strs[-1].split('.jpg')[0] + '_dino_' + str(round(rmse,4)) +'.jpg'
            pred_output_file = os.path.join('viz_depth/scannet', 'dino', img_name)
